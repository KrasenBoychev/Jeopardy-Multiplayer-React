...
export default function CreateQuestion() {
  ...
  const [checkIfQuestionExists] = useCheckIfQuestionExistsMutation();
  const [recordSingleQuestion] = useRecordSingleQuestionMutation();
  ...
  const handleNextPageAndSubmit = async () => {
    ...
    const questionNamesCheck = checkQuestionNames(
      formValues.question.content,
      createItemsAllValues,
      currentPage
    );
    if (questionNamesCheck.result) {
      toast.error(questionNamesCheck.error);
      return;
    }

    try {
      const response = await checkIfQuestionExists(
        formValues.question.content.trim()
      );

      if (response.data) {
        toast.error(formValues.question.content + " already exists");
        return;
      }
    } catch (err) {
      dispatch(
